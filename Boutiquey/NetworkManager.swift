...
class NetworkManager:NSObject{
    ...
    func callingHttpRequest(params:Dictionary<String,Any>, apiname:String,cuurentView:UIViewController,taskCallback: @escaping (Int,
        ...
        print("url",urlString)
        print("params", params)
        Alamofire.request(urlString,method: .post,parameters:params).validate().responseJSON { response in
...
//                                    imageView.image = img; profileImageCache.setObject(img, forKey: urlString as AnyObject)
//                                    imageView.backgroundColor = UIColor.white
//                                })
//                                    
//                                }
...
//    }
    ...
    func emailTest(emailText : String) -> Bool {
        let emailRegex = "[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}"
        let emailTest = NSPredicate(format: "SELF MATCHES %@", emailRegex)
        
        let testedEmailText = !(emailTest.evaluate(with: emailText))
        
        return testedEmailText
    }
